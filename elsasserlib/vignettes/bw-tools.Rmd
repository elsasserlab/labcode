...
# Now we bin it
binned.score <- bw_bins(bw.file, colnames=c('score'), bsize=10000)
...
Yes, not very informative, since signal at the first bins is zero. 
...
# Note: this should be a path for an actual file.
bw.file <- system.file("extdata", "example.1000bs.bw", package = "elsasserlib")
bw.file.2 <- system.file("extdata", "example.1000bs.bw", package = "elsasserlib")

# Now we bin it
# The files are actually the same, so we need to provide column names to avoid
# confusion:
binned.score <- bw_bins(c(bw.file, bw.file.2),
                        colnames=c('file1', 'file2'),
                        bsize=10000)
